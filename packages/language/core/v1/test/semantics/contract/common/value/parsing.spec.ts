...
describe("value", () => {
  it.each(["value"])("(%p) can be decoded/encoded and is isomorphic", async (filename) => {
    await pipe(
      TE.Do,
      ...
      TE.bindW("typed", ({ decoded }) => TE.fromEither(pipe(Value.decode(decoded), E.mapLeft(formatValidationErrors)))),
