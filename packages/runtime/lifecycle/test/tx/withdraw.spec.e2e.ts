          ...
          , TE.bindW('contractId',({runtime,adaProvider,tokenProvider,swapRequest,swapContract}) =>
              pipe( runtime(adaProvider).contracts.create
                  ...
                  , TE.chainW ((contractId) =>
                    runtime(adaProvider).contracts.applyInputs
                        (contractId)
                        ((next) => ({ inputs : [pipe(next.applicable_inputs.deposits[0],toInput)]})))
                  , TE.chainW (contractId =>
                    runtime(tokenProvider).contracts.applyInputs
                          (contractId)
                          ((next) => ({ inputs : [pipe(next.applicable_inputs.deposits[0],toInput)]}))))))
  ...
  it('Payouts can be withdrawn' , async () => {
    ...
    await
      pipe( executeSwapWithRequiredWithdrawalTillClosing
          , TE.bindW('result',({adaProvider,tokenProvider,contractId,runtime}) =>
              TE.sequenceArray(
                [ pipe
                  ( runtime(adaProvider).payouts.available (onlyByContractIds([contractId]))
                  , TE.map (payouts => { expect(payouts.length).toBe(1);return payouts.map(payout => payout.payoutId)})
